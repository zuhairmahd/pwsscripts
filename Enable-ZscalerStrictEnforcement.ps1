...
If (Test-Path $RegistryPath) {
    ...
    If ($RegistryValue -ne $disabled) {
        Write-Output 'Zscaler Strict Enforcement is already enabled'
    }
    else {
        Write-Output "The registry key $RegistryName is set to $RegistryValue. Changing to $enabled to enable Strict Enforcement."
        Set-ItemProperty -Path $RegistryPath -Name $RegistryName -Value $enabled
    ...
    New-ItemProperty -Path $RegistryPath -Name $RegistryName -Value $enabled -PropertyType DWORD -Force | Out-Null
    Write-Output 'Strict enforcement has been enabled.'
}
